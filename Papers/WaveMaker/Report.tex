...
\mathbf{u}_t)) \; (\nabla \cdot (h \mathbf{v}) )\: \mathrm{dx} \\ 
		...
		\begin{equation}
			...
			\subsection{Assessment of the code: the solitary wave test}	
				...
				For different time stepping methods and using or no a stabilization, we compute the solution and compare it to the initial condition at T=3.0s (during which the soliton moved of about 25 meters). 
				\begin{figure}[!h]
				\centering
					\begin{minipage}[t]{0.46\linewidth}
						\includegraphics[height=5cm]{ErrorL2.png}
						\caption{Error $\mathbb{L}^2$}
					\end{minipage}
					\begin{minipage}[t]{0.46\linewidth}
						\includegraphics[height=5cm]{ErrorL8.png}
						\caption{Error $\mathbb{L}^\infty$}
					\end{minipage}
				\end{figure}
